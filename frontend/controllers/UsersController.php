...
else if($insert_item_to_event==1)
{
...
$dataProvider = $searchModel->loadsearch(Yii::$app->request->queryParams,$slug);
	...
	{		
		...
		/*	$command = Yii::$app->DB->createCommand('select count(wvi.item_id) as stock FROM whitebook_vendor_item as wvi 
				LEFT JOIN whitebook_image as wi ON wvi.item_id = wi.item_id
				LEFT JOIN whitebook_vendor as wv ON wv.vendor_id = wvi.vendor_id 
				LEFT JOIN whitebook_category as wc ON wc.category_id = wvi.child_category 
				WHERE wvi.trash="Default" and wvi.item_approved="Yes" and wvi.item_status="Active" and wvi.type_id="2" 
				and wvi.item_for_sale="Yes" AND wi.module_type = "vendor_item" AND wvi.item_amount_in_stock > 0 
				AND wvi.item_id = "'.$data['item_id'].'" limit 1')->queryOne();*/						
	...
	{	
		if(Yii::$app->params['CUSTOMER_ID'] !='')
		{
		/*$order_sql = 'Select order_id from {{%order}} where customer_id='.CUSTOMER_ID;
		$order_cmd = Yii::$app->db->createCommand($order_sql)->queryAll();
		$order_tbl = Arrayhelper::getColumn($order_cmd,'order_id');
		
		foreach ($order_tbl as $key => $value) {
		$sub_order_sq = 'Select vendor_id,status_id from {{%suborder}} where order_id='.$value;	
		$sub_order_cmd= Yii::$app->db->createCommand($sub_order_sq)->queryAll();
		}		
			
		foreach ($sub_order_cmd as $key => $value1) {		
		$vendor_id = Arrayhelper::getValue($value1,'vendor_id');						
		$vendor = 'Select vendor_name from {{%vendor}} where vendor_id='.$vendor_id;	
		$vendor_name[] = Yii::$app->db->createCommand($vendor)->queryAll();		*/
