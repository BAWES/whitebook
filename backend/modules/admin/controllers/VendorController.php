   ...
   if ($model->load(Yii::$app->request->post())) {
			...
			if($model->save(false))		
				{						
					...
					Yii::$app->newcomponent->sendmail($vendor['vendor_contact_email'],$subject,$body,$message);						
				}
    ...
    {	
		...
		$k[]=$date;
		 ...
		 echo json_encode(array("date"=>$k,"count"=>$cnt,'input1'=>$as,'input2'=>$ae)); exit;
		 endif;
	}
    ...
    {	
	  ...
	  $data = Yii::$app->request->post();		  
		...
		echo json_encode(array("date"=>$k,"packid"=>$package_id,"start"=>$edit_start_date,"end"=>$edit_end_date,'input1'=>$as,'input2'=>$ae)); exit;
		 endif;
}
