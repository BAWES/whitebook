        ...
        <img src="'.$data->image.'" style="width: 100%;height: 100%;">
            ...
            'slide_url:url',
			 ...
			 [
			  ...
			  'format' => 'raw',
			  'value'=>function($data) {
				return HTML::a('<img src='.$data->statusImageurl($data->slide_status).' id="image-'.$data->slide_id.'" title='.$data->statusTitle($data->slide_status).'>','javascript:void(0)',['id'=>'status',
...
$this->registerJs("
	...
	function Status(status){
		...
		var pathUrl = '".Url::to(['/slide/status'])."';
	...
	function change_sort_order(sort_val,slide_id) {
		...
		if(sort_val != exist_sort || exist_sort == 0) {
			...
			{
				var csrfToken = $('meta[name=\"csrf-token\"]').attr('content');
				var path = '".Url::to(['/slide/sort_slide'])."';
