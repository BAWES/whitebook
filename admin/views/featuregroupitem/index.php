...
<p>
    ...
    [
			...
			[
             ...
             'format'=>'raw',                 
			  'value'=>function($data) {
				return HTML::a('<img src='.Yii::$app->newcomponent->statusImageurl($data->group_item_status).' id="image-'.$data->featured_id.'" alt="Status Image" title='.Yii::$app->newcomponent->statusTitle($data->group_item_status).'>','javascript:void(0)',['id'=>'status', 
...
<script type="text/javascript">
	...
	{		
        ...
        var path = "<?php echo Url::to(['/admin/featuregroupitem/block']); ?> ";
        $.ajax({  
        type: 'POST',      
...
function change_sort_order(sort_val,featured_id)
     {
		 ...
		 if(sort_val!=exist_sort || exist_sort==0)
		 {
			...
			{
				var csrfToken = $('meta[name="csrf-token"]').attr("content");		
				var path = "<?php echo Url::to(['/admin/featuregroupitem/sort_feature_group']); ?> ";
				$.ajax({  
				type: 'POST',      
				url: path, //url to be called
